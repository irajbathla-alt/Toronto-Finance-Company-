...
function doGet(e) {
  try {
    ...
    const actions = {
      ...
      adminUpdate, adminEnsureDrive, getDocuments
    };
...
function doPost(e) {
  try {
    ...
    const actions = {
      ...
      adminList, adminUpdate, adminEnsureDrive, getDocuments
    };
    if (!actions[p.action]) throw new Error('Unknown action');
...
function healthCheck() {
  const out = {
    ok:true,
    service:'Toronto Finance Company CRM Stable',
    lazyDriveFolders:true,
    approvalEmailEnabled:true
  };
  try {
    out.spreadsheetName = SpreadsheetApp.openById(CONFIG.SHEET_ID).getName();
...
function rows() {
  ...
  return values.filter(row => row[idIndex])
...
function clientLogin(p) {
  ...
  const record = rows().find(row =>
    String(row.email).trim().toLowerCase() === email && row.passwordHash === hash(p.password || '')
  );
...
function getDocuments(p) {
  const record = find(p.applicationId);
  return { ok:true, data:listDocs(record) };
}
...
function adminUpdate(p) {
  ...
  const nextStatus = Object.prototype.hasOwnProperty.call(patch,'status') ? String(patch.status) : String(before.status || '');
  ...
  if (APPROVAL_EMAIL_STATUSES.includes(nextStatus) && !alreadyNotified) {
    notification = sendApprovalNotification({ ...before, ...patch, status:nextStatus });
  }
