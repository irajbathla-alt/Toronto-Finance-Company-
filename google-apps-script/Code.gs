...
function rows(){const values=sheet().getDataRange().getValues();return values.slice(1).filter(r=>r.some(v=>v!==''&&v!==null)).map(r=>Object.fromEntries(HEADERS.map((h,i)=>[h,r[i]])));}
...
function update(id,patch){const sh=sheet(),data=sh.getDataRange().getValues();for(let i=1;i<data.length;i++){if(data[i][0]===id){HEADERS.forEach((h,j)=>{if(Object.prototype.hasOwnProperty.call(patch,h))data[i][j]=patch[h];});data[i][HEADERS.indexOf('updated')]=new Date();sh.getRange(i+1,1,1,HEADERS.length).setValues([data[i]]);return;}}throw new Error('Application not found');}
